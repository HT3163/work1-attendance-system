...
const AdminDashboard = () => {
    ...
    const onUpdate = async (id, status) => {
        try {
            ...
            setAttendance(attendance.map((record) => (record._id === id ? { ...record, status } : record)));
        } catch (err) {
            console.error(err.response.data);
        }
    };
    ...
    const generateSystemReport = async () => {
        try {
            const res = await API.post('/admin/system-report', { from: fromDate, to: toDate });
            setReport(res.data);
        } catch (err) {
            console.error(err.response.data);
        }
    ...
    const setGrade = async () => {
        try {
            const res = await API.put('/admin/grade', { userId: gradeUserId, attendedDays });
        ...
        <div className="dashboard">
            ...
            <div className="main-content">
                ...
                <div className="content">
                    ...
                    <h2 id="attendance">Manage Attendance</h2>
                            ...
                            <TableBody>
                                {attendance.map((record) => (
                                    <TableRow key={record._id}>
                                        ...
                                        <TableCell>
                                            <Button style={{ margin: "0 5px" }} variant="contained" color="primary" onClick={() => onUpdate(record._id, 'present')}>Mark Present</Button>
                                            <Button style={{ margin: "0 5px" }} variant="contained" color="secondary" onClick={() => onUpdate(record._id, 'absent')}>Mark Absent</Button>
                                            <Button variant="contained" style={{ backgroundColor: yellow[700], color: '#fff', margin: "0 5px" }} onClick={() => onUpdate(record._id, 'leave')}>Mark Leave</Button>
                                            <Button style={{ margin: "0 5px" }} variant="contained" color="error" onClick={() => onDelete(record._id)}>Delete</Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                    ...
                    <h2 style={{ marginTop: 20, marginBottom: 10 }}>Set User Grade</h2>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 3 }}>
                        <TextField
                            label="Roll Number"
                            variant="outlined"
                            value={gradeUserId}
                            onChange={(e) => setGradeUserId(e.target.value)}
                        />
                        <TextField
                            label="Attended Days"
                            type="number"
                        ...
                        />
                        <Button variant="contained" color="primary" onClick={setGrade}>
                            Set Grade
                        </Button>
                    </Box>
                </div>
